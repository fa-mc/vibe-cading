...
### NEXT (queued, not started) — the trapezoid socket on the two END walls

User direction at the round-50 checkpoint: the ±X side walls now carry the
trapezoid mating socket (round 50); the same feature is wanted on **the other
two walls** — the latch end (−Y) and the tongue end (+Y).

**Not yet verified against the reference.** Round 50 measured the socket only
on the ±X walls. Before building anything, ray-cast `25560.dat` across the end
walls the same way (`tmp/ldraw/` probes; the reference STL is regenerated by
`tmp/ldraw/dump_housing_stl.py`) and establish, per wall:

- whether a trapezoid exists there at all, and at what Y/Z;
- its own dimensions — do NOT assume the ±X numbers
  (`SOCKET_Y_HALF_LO/HI = 9.200/11.200`, `SOCKET_Z_LO = 22.000`) transfer, the
  end walls are a different thickness (latch 1.200/4.800, tongue varies) and a
  different height band;
- whether those walls thicken the way the side walls do at
  `WALL_INNER_STEP_Z = 21.200`, which is what makes the recess a pocket rather
  than a hole.

**The trap to avoid** is the one round 50 just corrected: the design doc's §4
described the side-wall step as full-length when it is local, and the model
copied that for ~35 rounds. Read the geometry, not the table.

**Known interactions.** The latch end wall carries the latch pocket and its
clearance (`_build_latch_wall`, `_build_latch_clearance`, `_build_latch_land`)
and the finger windows; the tongue end carries the rebate and the round-46
locating ribs. A socket at either end lands in an already-crowded wall, so
check seated interference against the Cover and re-run
`test_interior_clears_the_target_battery` — the end walls bound the pack's Y,
where it has only 2.4 mm of slack at each end.
