...
Run the orthographic preview pipeline for the model `$ARGUMENTS` exactly as documented in `CLAUDE.md` under "Asset Validation":
...
2. If a reference image or drawing has been attached or referenced in the conversation, follow the **Step 0 — establish orientation** procedure from `CLAUDE.md` *before* extracting any dimensions, then read each SVG with the `Read` tool and compare against the reference.
