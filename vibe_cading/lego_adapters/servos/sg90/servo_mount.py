...
from vibe_cading.lego.constants import PIN_HOLE_DIAMETER, STUD_PITCH
...
from vibe_cading.lego_adapters.servos.shaft import Shaft
from vibe_cading.rc.servo.sg90 import Sg90Servo
from vibe_cading.cq_utils import cut_at_positions, cylinder
from vibe_cading.lego_adapters._wall_helpers import orient_to_neg_x, orient_to_pos_x
from vibe_cading.mechanical.holes import CounterboreHole
from vibe_cading.print_settings import ToleranceProfile, FitGrade


# Zero-clearance profile used for the dimensioned countersunk screw holes in
# this case.  The geometry was reverse-engineered from a STEP reference with
# no manufacturing allowance baked into the constants (BOTTOM_SCREW_R = 0.80,
# BOTTOM_SCREW_CS_R = 1.43); using the part-default ``fdm_standard`` profile
# would silently inflate radii by ``free.radial`` (0.15 mm), which is large
# relative to a 0.8 mm bore.  Every grade is zeroed because downstream
# cutters read different grades depending on cut type.
_ZERO_TOL = ToleranceProfile(
    name="zero",
...
def _build_countersunk_screw_cutter(
    bore_r: float,
    bore_depth: float,
    cs_r: float,
    cs_depth: float,
    ...
    """Build a +Z-oriented BLIND countersunk-screw cutter at *center*.

    Adapter that produces the same geometry the legacy
    ``cq_utils.countersunk_hole`` returned: the wide end of the cone sits at
    Z = center.z and the bore extends *exactly* *bore_depth* mm in the +Z
    direction (no overshoot past the blind end).  Internally delegates to
    ``CounterboreHole(head_type='cone')`` to keep a single source of truth
    for screw-hole geometry.

    Why the explicit bounding box?
    ------------------------------
    ``CounterboreHole.to_cutter()`` deliberately extends both the entrance
    AND the bottom face by a large ``overcut`` so the cutter cleanly pierces
    a part whose far face matches the bore depth.  For a BLIND hole inside
    a part thicker than the bore, the bottom overcut would remove extra
    material past the intended floor.  We clip the cutter to a tight Z box
    [center.z - 0.01, center.z + bore_depth] to preserve the legacy blind
    semantics exactly.
    """
    # Derive head_angle so cone_h == cs_depth (matches legacy footprint).
    desired_angle = 2.0 * math.degrees(math.atan((cs_r - bore_r) / cs_depth))
    raw_cutter = CounterboreHole(
        shaft_diameter=bore_r * 2.0,
        shaft_depth=bore_depth,   # full hole depth — cone overlaps top region
        head_diameter=cs_r * 2.0,
        head_depth=cs_depth,
        head_type="cone",
        head_angle=desired_angle,
        profile=_ZERO_TOL,
    ).to_cutter()
    # CounterboreHole.to_cutter() is anchored at Z = 0 extending −Z; mirror so
    # it extends +Z, then translate to the call-site origin.
    cutter = raw_cutter.mirror("XY").translate(center)

    # Clip to preserve BLIND semantics: entrance gets a small overcut (0.01 mm
    # above Z = center.z) for clean entry, blind floor sits exactly at
    # Z = center.z + bore_depth.
    cx, cy, cz = center
    box_side = max(cs_r, bore_r) * 4.0
    z_lo = cz - 0.01
    z_hi = cz + bore_depth
    bound = (
        cq.Workplane("XY", origin=(cx, cy, z_lo))
        .rect(box_side, box_side)
        .extrude(z_hi - z_lo)
    )
    return cutter.intersect(bound)
...
class ServoMountBase:
    ...
    def _cut_servo_cavity(self, part: cq.Workplane) -> cq.Workplane:
        ...
        # Default ``Sg90Servo`` is constructed with ``cutter_clearance=0.2``
        # (the historical default); the new ``to_cutter(profile=None)``
        # signature reads that clearance off ``self.cutter_clearance``.
        servo_cutter = (
            servo_ref.to_cutter()
    ...
    def _cut_pin_holes(self, part: cq.Workplane) -> cq.Workplane:
        ...
        from vibe_cading.lego.cutters.technic_pin_hole import TECHNIC_PIN_CB_DIAMETER, TECHNIC_PIN_CB_DEPTH
    ...
    def _cut_bottom_screw(self, part: cq.Workplane) -> cq.Workplane:
        ...
        cutter = _build_countersunk_screw_cutter(
    ...
    def _cut_top_holes(self, part: cq.Workplane) -> cq.Workplane:
        ...
        from vibe_cading.cq_utils import cylinder
        from vibe_cading.lego.cutters.technic_pin_hole import TECHNIC_PIN_CB_DIAMETER
    ...
    def _cut_clamp_lock(self, part: cq.Workplane) -> cq.Workplane:
        ...
        from vibe_cading.mechanical.joints import DovetailJoint
        ...
        # DovetailJoint bakes a 1 mm entry overlap into ``to_cutter`` — the
        # host body's external face sits well past this, so volume parity holds.
        socket_cutter = joint.to_cutter()
...
class ServoMountClamp:
    ...
    def _build(self) -> cq.Workplane:
        ...
        tab_cutter = _build_countersunk_screw_cutter(
            bore_r=0.8,
            bore_depth=6.0,
            cs_r=1.43,
            cs_depth=0.5,
