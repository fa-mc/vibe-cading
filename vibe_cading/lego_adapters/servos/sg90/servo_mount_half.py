...
from vibe_cading.cq_utils import rounded_box, cut_at_positions, cylinder
...
# Zero-clearance profile used for the dimensioned countersunk screw hole in
# this case.  See the matching note in ``servo_mount.py`` — the legacy STEP
# reference baked clearance into the constants, so the migration from the
# old ``cq_utils.countersunk_hole`` helper must opt out of the
# ``CounterboreHole`` profile inflation to preserve geometry parity.
...
def _build_countersunk_screw_cutter(
    ...
    Adapter producing the same geometry the legacy
    ``cq_utils.countersunk_hole`` returned: the wide end of the cone sits at
    Z = center.z and the bore extends *exactly* *bore_depth* mm in +Z (no
    overshoot past the blind end).  See the matching adapter in
    ``servo_mount.py`` for the full rationale on the bounding-box clip.
    """
    desired_angle = 2.0 * math.degrees(math.atan((cs_r - bore_r) / cs_depth))
    raw_cutter = CounterboreHole(
        shaft_diameter=bore_r * 2.0,
        shaft_depth=bore_depth,
