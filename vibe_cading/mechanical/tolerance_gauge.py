...
class ToleranceGauge:
    ...
    def _build(self) -> cq.Workplane:
        ...
        for i, offset in enumerate(self.offsets):
            ...
            # 1. Row 1 (y_coords[0]): M3 Clearance — sweep the
            # ``free.radial`` allowance per offset.  Phase 5 dropped
            # ``radial_allowance`` / ``head_recess_depth`` kwargs on
            # ``MetricMachineScrew.to_cutter`` in favour of full-profile
            # plumbing, so we build a single-grade tolerance profile per
            # column with the offset on ``free.radial`` and 1.0 mm head
            # recess on ``free.axial`` (matching the pre-Phase-5 literal).
            # ``ToleranceProfile`` / ``FitGrade`` re-imported in the next
            # block for the bearing-pocket row; kept local-import there
            # for parity with the original structure.
            from vibe_cading.print_settings import ToleranceProfile as _TP, FitGrade as _FG
            m3 = MetricMachineScrew.from_size("M3", length=12.0, head_type="socket")
            m3_prof = _TP(
                name=f"gauge_offset_{offset}",
                free=_FG(radial=offset, axial=1.0),
                slip=_FG(radial=0.0, axial=0.0),
                press=_FG(radial=0.0, axial=0.0),
            )
            m3_cutter = m3.to_cutter(profile=m3_prof, fit="clearance")
            ...
            brg = Bearing(inner_diameter=5.0, outer_diameter=8.0, thickness=2.5)
            from vibe_cading.print_settings import ToleranceProfile, FitGrade
            # ``Bearing.outer_pocket`` reads ``profile.press.radial``, so put
            # the swept offset on that grade's radial slot; the rest are zero.
            p = ToleranceProfile(
                name="test",
                free=FitGrade(radial=0.0, axial=0.0),
                slip=FitGrade(radial=0.0, axial=0.0),
                press=FitGrade(radial=offset, axial=0.0),
            )
