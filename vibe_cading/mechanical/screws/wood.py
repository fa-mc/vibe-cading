...
class WoodScrew:
    ...
    def to_cutter(self, profile=None, fit: str = "clearance") -> cq.Workplane:
        """Boolean-subtraction tool for the wood screw.

        :param profile: Optional :class:`ToleranceProfile`.  Forwarded to
            the underlying :class:`CounterboreHole`.
        :param fit: ``"clearance"`` (free-pass), ``"tap"`` (pilot bore
            for the threaded tip), or ``"interference"`` (press fit).
        """
        if fit == "clearance":
            ...
            raise ValueError(
                f"WoodScrew to_cutter fit must be 'clearance', 'tap', or "
                f"'interference', got {fit!r}"
            )
