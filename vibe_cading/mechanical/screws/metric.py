...
class MetricMachineScrew:
    ...
    def solid(self) -> cq.Workplane:
        ...
        if self.head_type == "flat":
            ...
            if self.drive is not None:
                res = res.cut(self.drive.to_cutter())
            return res
            ...
            if self.head_type == "socket":
                ...
                try:
                    ...
                    pass
            if self.drive is not None:
                head = head.cut(self.drive.to_cutter().translate((0, 0, self.head_height)))
    ...
    def to_cutter(self, profile=None, fit: str = "clearance") -> cq.Workplane:
        ...
        :param profile: Optional :class:`ToleranceProfile`.  Forwarded to
            the underlying :class:`CounterboreHole` which reads
            ``profile.free.radial`` (shaft inflation) and
            ``profile.free.axial`` (head recess depth).
        :param fit: ``"clearance"``, ``"tap"``, or ``"interference"`` —
            selects the shaft diameter from the constructor data per
            Round 4 Q3 (call-site fit; constructor stays catalog-neutral).
        """
        # Determine the target shaft diameter based on fit
        if fit == "clearance":
        ...
        # B1 (TL review 2026-05-14): mirror the sibling fallback pattern so a
        # caller that uses the protocol's documented default (``profile=None``)
        # does not crash inside CounterboreHole at the ``profile.free.radial``
        # dereference.  This matches WoodScrew / PlasticsScrew / SetScrew /
        # ImperialMachineScrew.  Geometry-neutral when build.toml-routed callers
        # already plumb a profile; restores docs/screws.md canonical example.
