...
from vibe_cading.cq_utils import rounded_box, cylinder
...
class Sg90Servo:
    ...
    # ── Default cavity-cutter clearances (Phase 4: baked per-class) ───────────
    # The Sg90 ``to_cutter`` was historically called with ``clearance=0.2``
    # and ``extend_shaft_up=5.0``; both numbers are now class-level
    # constants overridable at construction time so the call-time
    # signature can collapse to ``to_cutter(profile=None)`` per
    # CutterProtocol.  ``profile`` is accepted but does not currently
    # affect output since the cavity clearance is a geometric (not
    # manufacturing) concern.
    DEFAULT_CUTTER_CLEARANCE: float = 0.2     # +mm offset around body/collar/boss/shaft
    DEFAULT_EXTEND_SHAFT_UP: float = 5.0      # extra +Z shaft length for through-cut unions
    ...
    def __init__(
        ...
        cutter_clearance: float = DEFAULT_CUTTER_CLEARANCE,
        extend_shaft_up: float = DEFAULT_EXTEND_SHAFT_UP,
    ):
        ...
        # Cavity-cutter parameters — overridable at construction time so the
        # call-time ``to_cutter(profile=None)`` signature stays uniform.
        self.cutter_clearance = cutter_clearance
        self.extend_shaft_up = extend_shaft_up
    ...
    def to_cutter(self, profile: ToleranceProfile | None = None) -> cq.Workplane:
        ...
        Includes the main body, collar, gear boss, and shaft stub,
        expanded by ``self.cutter_clearance``.  The main body extends
        far downwards (``z_min = -20``) so the cutter cleanly removes
        material from any open-bottom host.

        :param profile: Currently unused — cavity clearance is a
            geometric (not manufacturing) concern owned by the
            ``cutter_clearance`` / ``extend_shaft_up`` constructor
            arguments.  Accepted to satisfy ``CutterProtocol``.
        """
        clearance = self.cutter_clearance
        extend_shaft_up = self.extend_shaft_up
        ...
        # Gear boss — unify the height with the collar height in the cutter.
        # The boss is physically 0.3mm shorter (3.9 vs 4.2), but preserving
        # that gap as a negative cavity would leave an unprintable 0.3mm
        # stair-step crescent on the ceiling of the host mount.
        gear_boss = cylinder(
