...
Design intent
-------------
A plain cylinder sized to press-fit-house one MR85-2RS ball bearing
(5 x 8 x 2.5 mm ID x OD x width), reusing
:meth:`vibe_cading.mechanical.bearings.Bearing.outer_pocket` (design brief
D4/D6) rather than hand-rolling a clearance-added circle.  No register/spigot
...
class BearingHexHousing:
    ...
    profile : str | None
        ...
        ``press`` fit grade, radial + axial (design brief D6) — a
        standalone housing insert, not intended to be user-serviceable.
    """
    ...
    def _build(self) -> cq.Workplane:
        ...
        # ── 2. Bearing pocket, press-fit clearance, through-cut ──────────
        # Bearing.outer_pocket() extrudes one-sided from Z = 0 to
        # Z = thickness + press.axial (bearings.py:88-91).  Applied
        ...
        # faces are functionally entry faces), centre the press.axial
        # overcut symmetrically across both faces by translating the
        # cutter down by half the axial allowance before cutting
        # (design brief D4).
        pocket = bearing.outer_pocket(profile=self._prof).translate(
            (0, 0, -self._prof.press.axial / 2.0)
        )
