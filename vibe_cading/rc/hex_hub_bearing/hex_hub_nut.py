...
with a through-bore for a shaft/axle to pass through.  No register/spigot
...
which ``.union()``s it flush against a
...
class HexHubNut:
    """12 mm hex wheel-hub adapter nut with a free-fit through-bore.
    ...
    profile : str | None
        ...
        uses the globally-configured default.  The through-bore uses
        ``free`` fit grade (design brief D5) — a shaft/axle passes
        through loosely, no press/snug-fit function.
    """
    ...
    def _build(self) -> cq.Workplane:
        ...
        # in the bottom / top plane -- this includes both the 6 hex
        # perimeter edges *and* the through-bore's circular entry edges
        # (the bore is cut through-all, so its ends sit exactly at Z = 0
        # and Z = thickness too).  This mirrors
        # `vibe_cading.rc.freespin_hex_hub.FreespinHexHub`'s identical
        # chamfer pattern, and is a deliberate, print-friendly side effect,
        # not an oversight: it adds a small lead-in chamfer to the bore
        # openings.  It only touches the outermost 0.5 mm of bore depth --
        # the bulk mid-thickness bore diameter (what Test 3 measures) is
