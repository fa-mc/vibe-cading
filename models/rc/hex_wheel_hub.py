...
class HexWheelHub:
    ...
    def _pocket_dia(self) -> float:
        ...
        ``free_fit`` is a *radial* value (×2 for diameter).  This gives
    ...
    def _build(self) -> cq.Workplane:
        ...
        # bore_dia defaults to 6 mm for a 5 mm shaft (0.5 mm radial clearance).
        part = (
            part
            .faces("<Z")       # Z = 0, outward normal → −Z
            .workplane()
            .circle(self.bore_dia / 2.0)
            .cutThruAll()
        )
        ...
        assert len(part.solids().vals()) == 1, (
            "HexWheelHub: expected a single contiguous solid — check pocket "
