...
- **Manufacturing & Tolerance Profiles:** Never hardcode a "magic" clearance (like `+ 0.2`) deep inside an internal boolean cut or method. Tolerances should map to user-maintained profiles (e.g., `fdm_standard`, `resin_precise`, `cnc`). Subtractive classes/methods must accept a `clearance` parameter or a profile object, allowing users to inject their own manufacturing offsets without rewriting the core component logic.
