...
- **Branch-Deletion Policy (Three-Clause):** Branch deletion is gated by *merge state*, not by the verb — the first clause that matches decides. (Branch deletion is distinct from the Force-Push Policy above; force-push / history-rewrite stay governed by that policy.)
  1. **NEVER delete the stable base branch (`main`) or a shared long-lived branch.** Every consumer clones / branches from these; deleting one is destructive across the whole project regardless of merge state. Hard restriction, no per-instance override.
  2. **Confirm-first (explicit current-turn ask) for an *unmerged* branch** — or any branch whose merged-ness you cannot verify. Deleting unmerged work risks irrecoverable loss, and `git branch -D` forces past git's own safety check, so the human must authorise it in the current turn.
  3. **Auto-granted (no confirmation) for a feature branch *verified fully merged* into `main`** — i.e. merging it into `main` would add nothing. A `MERGED` PR (`gh pr view <branch> --json state`) is a **necessary precondition but NOT sufficient on its own** — a commit can land on the branch's local tip *after* the merge (e.g. a shared-worktree IDE auto-commit) and the PR signal will not reflect it. So **before every autonomous delete, run the tree-equality check**: `git merge-tree --write-tree main <branch>` must equal `git rev-parse main^{tree}` (merging the branch into `main` produces `main`'s exact tree, so the branch adds nothing). If they differ, the branch carries un-landed content — fall back to clause 2 (confirm-first). This is non-destructive cleanup (the merged content is preserved on `main`); it applies to local *and* remote branches and is a standard wrap-up hygiene step; for a **remote** branch, also confirm no collaborator is still building on it. **Do NOT decide merged-ness with commit-ancestry or plain-diff checks — under squash-merge they all misfire:** `git branch -d` mis-reports the branch as "not fully merged" (the original commits are not ancestors of `main`); `git cherry main <branch>` mis-flags it (it matches per-commit *patch-ids*, which a squash collapses); and a plain `git diff main <branch>` false-alarms as soon as `main` advances independently. Once verified, delete with `-D`.
...
- **Detect-and-Surface a Needed Reframe (Frame-Check):** Choosing to reframe the *direction* of an investigation is the human's call; *detecting* that a reframe may be needed and *surfacing* it is yours — do not silently stay frame-locked. **Trigger:** your next move is to control, normalize, or equalize an observed anomaly — or to footnote it as a caveat — so you can keep pursuing the current question. Pause: the anomaly may answer a *more important* question than the one you are on; surface it as an explicit fork to the human rather than setting it aside. **Two mechanical tells** it is firing: (a) you wrote a caveat of the form "this does NOT address / validate Y" and Y is plausibly the user's real concern — surface Y as a fork, do not bury it; (b) the options you are about to present are all mechanism-variations within one frame, none questioning the frame — add the framing question. **Scope guard (avoid noise):** surface only when the set-aside thing is plausibly *more important* than the current question — not for every minor caveat. Distinct from the **Tunnel Vision Circuit-Breaker** above: that targets code retry-loops (you keep editing the same file); this targets *frame* lock-in (you keep investigating within a premise that may itself be wrong). *(Triggering incident: a cross-host determinism investigation held a "make the inputs byte-equal to isolate the hardware variable" frame through a pre-gate failure, caveated "this does NOT validate data correctness," then offered a mechanism-only menu that omitted the correctness question — which was THE question: one host's inputs were *wrong*, not merely *different*. The human supplied the reframe; the frame-check is the discipline that surfaces it first.)*
...
- **Role Activation Protocol:** Whenever a transition into a named role is required — whether triggered by the host's subagent mechanism, a slash command, or an explicit `@role` mention in chat — read the persona file before adopting it. Do not infer the persona from memory or prior context.
  ...
  - **`@pm` (not project-tracked):** This repository intentionally does not ship a PM persona. An inline `@pm` mention is a signal to escalate to the human contributor, who drives backlog prioritisation by default. If a maintainer has loaded their own PM persona via their host's user-config mechanism (for Claude Code: `~/.claude/`), hand off to that persona instead. Do not fabricate a PM persona from memory.
...
- **Orchestrator-Brain Routing — Delegate by Default, Size Every Seat Explicitly:** The orchestrating session runs on a high capability tier and its context is the scarce resource (*Conserve Context Window* below is the *why*). It therefore does only the thinking that cannot be delegated — task sequencing, gate decisions, disposition of review findings, commits/pushes, and user-facing reports — and **delegates everything substantive to subagent seats, actively and by default**: fresh-context and adversarial reviews, design-brief authoring, implementation against a brief, log and artifact scans, multi-file searches, STEP-analysis digests. Spawning is authorized in general; it is never a permission event, and it never satisfies a human-gated sign-off. *(Why: an unnamed tier silently inherited the orchestrator's model on routine scans — lineage entry 2026-09-02.)*
  - **Name the tier on every non-role spawn.** Three tiers suffice: **strong** for adversarial / fresh-context review, design judgment, and anything integrity-critical; **mid** for research, drafting, mechanical fan-out, and implementation against an approved brief; **cheap** for search and scan. An omitted tier silently inherits the orchestrator's — the most expensive one — spent on work this rule routes lower for zero quality gain, and invisible because the spawn succeeds either way. An integrity-critical non-role seat is always named **strong**; raising a seat to a tier your host places *above* strong (if it offers one) is a human-gated, per-seat grant, never the author's option. Your host's instruction file defines the tier → concrete-model mapping (for Claude Code: `CLAUDE.md` § *Tier → model mapping (Claude Code)*).
  - **Role seats are the inverse: pass nothing.** A spawn of one of this project's four shipped roles is governed by that role's own persona file; do not override its tier at the call site. Escalate such a seat by spawning a *separate* seat, never by re-pointing the pinned one.
  - **Escalation is mid → strong, on signal, same brief — for WORK seats only.** When a mid-tier work seat (research, drafting, scan, implementation) returns low-confidence, contradictory, or shallow findings on a correctness-critical task, re-spawn on the strong tier with the same brief **plus a pointer to the first attempt's artifact**. A weak *review* seat is never escalated this way — it is re-spawned fresh per *Fresh-context isolation for review subagents* above.
  - **Disposition of review findings is the one inline exception:** the orchestrator applies the fix itself (without spawning a seat) only when it is a reviewer-enumerated one-clause edit **and touches no file the orchestrator has not already read this turn**; otherwise spawn a seat.
- **Drafted External Comments — Post in One Step:** When a role drafts a GitHub issue or PR comment to address a user-directed task, post it in the same turn rather than gating on a separate *"approve to post"* step. Drafting and posting are one action; the user keeps revert authority via the GitHub UI. Opening a pull request for already-authorized work is likewise one action with the branch and commits it carries — open it in the same turn, with no separate "approve to open" gate (the user retains revert authority via the GitHub UI, and the *merge* stays the human's call). Still requires explicit approval: opening a new issue, mentioning users not already in the thread, comments touching security-sensitive content, or any post to a destination the user has not already pointed the role at.
- **PR-Review Follow-ups — Address Inline in Same PR:** When a reviewer (fresh-context subagent, an automated review pass, inline GitHub comment, multi-role review, or the human) raises a follow-up on an open PR before merge, address it **inline on the same PR branch** as additional commits — do NOT defer it to a separate `TODO.md` row or a follow-up PR. The default is fix-now-on-this-branch; deferral is the exception and must be earned against one of the carve-outs below. *Why:* TODO rows accumulate and rot; inline application gets the fix reviewed and merged in the same cycle, on the same context, by the same reviewers who flagged it. Carve-outs where deferral IS appropriate:
...
- **Instruction-Graph Changes — Lightweight Path, Mandatory Fresh-Context Review:** Changes to the *instruction graph itself* (the root `AGENTS.md`, `vibe/INSTRUCTIONS.md`, `vibe/INSTRUCTIONS-lineage.md`, `vibe/agents/<role>.md`, `vibe/commands/`, `vibe/templates/`, `docs/agentic-workflow.md`, the active host instruction file) are a documented exception to the full design flow: they need **no** requirements or design-brief artifact — a user-directed instruction change already satisfies the direction gate, and the intermediary artifacts would only duplicate provenance the instruction graph already records. What does **not** degenerate is **independent review**: cold-start coherence is this graph's quality bar and an author is structurally blind to what a cold reader misses. Therefore a **new rule, a new role/command/template file, or a section reshape** requires a **fresh-context review subagent** on the drafted diff **before commit** (spawned under *Fresh-context isolation for review subagents* and *No self-review for integrity sign-offs* above — the reviewer must not be the drafter). **In-place wording, typo, and pointer fixes that introduce no new behavior** may be committed directly. Policy-level changes (autonomy, review gates, licensing) still require human sign-off before landing. *(Why: this rule's own introducing diff took three fresh-context BLOCK rounds to become cold-start coherent — lineage entry 2026-09-02.)* **Provenance placement for new rules:** a rule added to this file from 2026-09-02 on keeps at most **one line** of *why* (a one-line triggering-incident citation) inline; the incident narrative, directives, and superseded history go to [vibe/INSTRUCTIONS-lineage.md](INSTRUCTIONS-lineage.md) under a heading matching the rule's bold name, in the same commit. Existing inline narratives are **not** retro-extracted.
...
- **Principled-Over-Expedient Path Default:** For a path/routing choice between a more principled option and a more expedient one — independent fresh-context review vs in-place review, full Designer design-flow vs a direct code edit, isolated branch + PR vs editing `main` in place, writing a requirements/design brief first vs jumping straight to implementation (except instruction-graph changes — see *Instruction-Graph Changes* above) — default to the more principled path whenever the cost gap is small (rule of thumb: ≤ ~1 hour extra wall-clock, no material new resource cost). Declare the choice and proceed; do not pause to ask. Pause and surface the choice only when ANY of these fire: (a) a materially large cost gap; (b) an irreversible action; (c) genuinely ambiguous scope; (d) a choice affecting shared state; (e) an explicit human gate (e.g. design-brief user approval). The user already made the principled-vs-expedient meta-call by adopting this workflow — do not re-litigate it per decision.
- **Conserve Context Window:** Treat the orchestrator's token budget AND the user's per-message cognitive load as scarce resources; at every routing and status-surface choice, prefer the leaner path. Concretely: (1) when a step needs verbose tool output — multi-file reads, log dumps, repeated STEP-analysis runs, more than ~2 chained tool calls — spawn a subagent to absorb it rather than carrying the raw output in the orchestrator (the Subagent Outcome Discipline above is the mechanism); (2) when extending an instruction or doc file, prefer a ≤1-line pointer to the authoritative section over restating its content inline; (3) when pinging the user at a decision point, give field-structured, <30-second-scannable status — never a dense ID dump or jargon-laden subagent summary passed through unmodified. These are positive design defaults, not anti-patterns to avoid. *Earns-its-keep exceptions:* cold-start rules every agent reads once per session, a **one-line** citation of the triggering incident that explains why a rule exists, and structured summary fields that make a status ping more scannable — keep these even though they cost tokens. The full incident narrative behind a rule lives in [vibe/INSTRUCTIONS-lineage.md](INSTRUCTIONS-lineage.md), which nothing loads at cold start — see *Instruction-Graph Changes* above for the authoring rule. Narratives already inline in this file predate that split and stay as they are; do not retro-extract them (each move risks silently dropping a clause).
...
- **Object-Oriented Component API:** Mechanical joints, hardware wrappers, and modular utilities should be designed as Python classes rather than bare functions. Standardize boolean interfaces on the project's real contract: a uniform `.solid` property for read-only positive geometry (every positive-geometry generator exposes it; pure-cutter producers such as the `FastenerDrive` family expose only `.to_cutter()`), and `.to_cutter(profile=None)` for the subtractive cutter side (the universal `CutterProtocol` in `vibe_cading/mechanical/protocols.py`). Joints additionally expose `.male(overlap: float)` as a joint-specific positive accessor (`JointProtocol` in `vibe_cading/mechanical/joints/protocol.py`); there is **no** `.female()` method — the subtractive side is always `.to_cutter()`.
...
4. If you need to interact with GitHub (e.g. checking issues or PRs via `gh`), authenticate by sourcing the `.env` file so `gh` picks up `GH_TOKEN` from the environment (e.g., `set -a && . ./.env && set +a && gh ...`); do not extract it with `export $(grep … | xargs)` (word-splits values with spaces/quotes) and do not run `gh auth login`.
...
`cq.Workplane().box(...).cut(joint.to_cutter())`), or a `to_cutter()` overlay
...
### Implicit `extrude()` direction on a recess cutter

**Symptom:** A recess, counterbore, or pocket that should sink *into* a part sits on the wrong side of the entry face and removes almost nothing — the shoulder never forms, the bore stays at shaft diameter for the full wall, and the head gets no clearance. The part still builds, still passes `assert len(result.solids().vals()) == 1`, and still looks right in every external view, because the missing material is on the *inside*. Where the wrong side is open air the failure is pure under-removal; where it holds material, the same slip over-removes instead.

**Root cause:** `cq.Workplane.extrude(d)` extrudes along the workplane's **+normal** (+Z for `"XY"`) when `d` is positive, and that default is invisible at the call site: `cq.Workplane("XY", origin=(0, 0, z_entry)).circle(r).extrude(depth)` *reads* as "a recess of `depth` at `z_entry`" but spans `[z_entry, z_entry + depth]` — the wrong side whenever the material to remove lies *below* the entry face. An **offset** origin (e.g. `z_recess = -tolerance.free.axial`) hides it best, because attention goes to computing the entry Z and the direction is then never stated at all — but the offset only conceals the bug, it does not cause it: under a profile whose `free.axial` is `0.0` (the shipped `cnc`) the origin sits exactly at Z=0 and the defect is identical.

**Fix:**
1. **Anchor the workplane at the far end, or extrude a negative amount.** `cq.Workplane("XY", origin=(0, 0, z_entry - depth)).circle(r).extrude(depth)` puts the solid in `[z_entry - depth, z_entry]`. Whichever form you use, confirm *which side of the entry face the solid lands on* — that is the load-bearing fact, and the one thing neither the expression nor a reading of it makes visible.
2. **Cross-check the sibling branch.** Where one method builds the same recess two ways, the recess body must occupy the **same side of the entry face in both**, whatever primitive builds it (`extrude`, `loft`, revolve). A side that differs between two branches of one `if` means one is wrong.
3. **Assert WHERE the recess is, not merely THAT it exists.** A recess on the wrong side still has the right radius and volume — only its position is wrong — so any check blind to location passes it (*A Check That Cannot Fail Is Not A Check*, §4). A presence probe is fine **provided its location is one only correct geometry can occupy**. Expect the guard's shape to differ per accessor: a bare `zmax <= eps` reads clearly on a `.solid`, but on a `.to_cutter()` the through-overcut lifts the baseline to `_THROUGH_OVERCUT`, so the same form would have to hard-code that unrelated constant to stay meaningful — probing the recess annulus avoids the coupling. (The directional signal itself survives the overcut: pre- and post-fix `zmax` differ by exactly `head_depth`. This is about guard robustness, not detectability.)

*(Triggering incident: `CounterboreHole` (`vibe_cading/mechanical/holes.py`) carried this **twice, independently** — in `.solid` and again in `.to_cutter()`. One class making the identical slip in two methods is what marks it a footgun, not a typo. Note what was actually missing: the branch was **not** unexercised — `ToleranceGauge` had been driving an M3 socket-head cutter through it all along, and the fix moved that part's volume by 246.58 mm³. It lacked a *guard*, not a consumer. Guards now: `tests/test_cutter_overcut.py::test_counterbore_hole_cylinder_head_sinks_into_part` (bbox) and `::test_counterbore_hole_to_cutter_cylinder_head_sinks_into_part` (annulus probe) — the two shapes in Fix 3.)*
