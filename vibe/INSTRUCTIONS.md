...
### Implicit `extrude()` direction on an offset-origin recess

**Symptom:** A recess, counterbore, or pocket that should sink *into* a part instead sits in open air just outside the entry face. The cut removes almost nothing — or, where the recess is wider than the shaft it should step back down to, the oversized bore punches straight through the full wall thickness instead of forming a shoulder. The part still builds, still passes `assert len(result.solids().vals()) == 1`, and still looks correct in every external orthographic view, because the material that should have been removed is on the *inside*.

**Root cause:** `cq.Workplane.extrude(d)` extrudes along the workplane's **+normal** (for an `"XY"` workplane, +Z) when `d` is positive. That default is invisible at the call site: `cq.Workplane("XY", origin=(0, 0, z_entry)).circle(r).extrude(depth)` *reads* as "a recess of `depth` at `z_entry`" but actually builds the solid spanning `[z_entry, z_entry + depth]` — the wrong side whenever the material to be removed lies *below* the entry face. The slip hides especially well when the origin is **offset** rather than sitting at the workplane's own Z=0 (e.g. `z_recess = -tolerance.free.axial`): the reader's attention goes to computing the correct entry Z, and the direction is then never stated anywhere in the expression.

**Fix:**
1. **State the direction explicitly whenever the origin is offset.** Either anchor the origin at the *far* end and extrude back toward the entry face — `cq.Workplane("XY", origin=(0, 0, z_entry - depth)).circle(r).extrude(depth)` — or extrude a negative amount. Never lean on the default when the entry face is not the workplane's own Z=0.
2. **Cross-check against the sibling branch.** If one branch of the same method already gets it right (`CounterboreHole`'s `"cone"` / flat-head branch sank correctly the whole time), the branches must agree. Two branches of one `if` that extrude in opposite directions means one of them is a bug.
3. **Assert the recess's Z *extent*, not its presence.** A recess that floated entirely outside the part still has the right radius and the right volume — only its position is wrong — so any presence-only or volume-only check passes it (*A Check That Cannot Fail Is Not A Check*, §4). Assert the Z band directly. Note that a bounding-box check is only sufficient when nothing else stretches the bbox: it works on a `.solid`, but on a `.to_cutter()` the through-overcut (100 mm) swamps `zmax` regardless of which way the recess points, so probe for material **presence at a point inside the recess annulus** (between shaft and head radii, strictly below the entry face) instead.

*(Triggering incident: `CounterboreHole` (`vibe_cading/mechanical/holes.py`) carried this defect **twice, independently** — once in `.solid` and again in `.to_cutter()` — and both survived until PR #94 gave the cylindrical-head branch its first real consumer (`Arrma223sEscMount`). One class making the identical directional slip in two separate methods is what marks this as a genuine footgun rather than a one-off typo. The durable guards are `tests/test_cutter_overcut.py::test_counterbore_hole_cylinder_head_sinks_into_part` (bbox `zmax` on `.solid`) and `::test_counterbore_hole_to_cutter_cylinder_head_sinks_into_part` (annulus point-probe on the cutter) — the two shapes described in Fix item 3.)*
