...
- **Branch-Deletion Policy (Three-Clause):** Branch deletion is gated by *merge state*, not by the verb — the first clause that matches decides. (Branch deletion is distinct from the Force-Push Policy above; force-push / history-rewrite stay governed by that policy.)
  ...
  3. **Auto-granted (no confirmation) for a feature branch *verified fully merged* into `main`.** The authoritative test is **`git diff main <branch>` is empty** — i.e. every change on the branch is already on `main`. A `MERGED` PR is the common signal but is *not* sufficient on its own: commits can be pushed to a branch after its squash-merge, so always confirm with the diff. This is non-destructive cleanup (the merged content is preserved on `main`); it applies to local *and* remote branches and is a standard wrap-up hygiene step. For a **remote** branch, also confirm no collaborator is still building on it. **Squash-merge caveat — do not use commit-ancestry checks here:** because this project squash-merges, `git branch -d` mis-reports a squash-merged branch as "not fully merged" (the original commits are not ancestors of `main`), and `git cherry main <branch>` mis-flags it as unmerged (it matches per-commit *patch-ids*, which a squash collapses into a new one). Use the `git diff main <branch>` content check above, *then* delete with `-D`.
