...
- **Branch-Deletion Policy (Three-Clause):** Branch deletion is gated by *merge state*, not by the verb — the first clause that matches decides. (Branch deletion is distinct from the Force-Push Policy above; force-push / history-rewrite stay governed by that policy.)
  ...
  3. **Auto-granted (no confirmation) for a feature branch *verified fully merged* into `main`** — its PR shows `MERGED`, or `git cherry main <branch>` / a content check confirms every change landed. This is non-destructive cleanup (the commits are preserved in the merge / squash commit), applies to local *and* remote branches, and is a standard wrap-up hygiene step. **Squash-merge caveat:** because this project squash-merges, `git branch -d` reports a squash-merged branch as "not fully merged" (its original commits are not ancestors of `main`); confirm merged-ness via the PR-merged / content check, *then* delete with `-D`.
