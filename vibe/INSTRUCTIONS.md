...
- **Deep-Modules — Dual-Lens Rule:** When applying the deletion test (`core-agents:structural-optimization` skill) to decide whether a class, base, or module earns its keep, evaluate BOTH lenses: (a) *maintainer-locality* — do current internal callers benefit from the abstraction (polymorphic dispatch, shared helpers, `isinstance` checks)? AND (b) *contributor-locality* — would an external OSS contributor adding a new family member benefit from inheriting / implementing this contract (IDE auto-completion, `@abstractmethod` enforcement, documented protocol shape)? The skill's canonical false-positive carve-outs (observability / versioning / security-boundary / cold-start-orientation / **contributor-extension contract**) — five as of core-agents v1.14.0, which lifted the contributor-extension carve-out upstream from this very project's pre-OSS structural pass. When an abstract contract has drifted from concrete implementations, prefer **repair** (re-align signatures) or **replace with `typing.Protocol`** over **remove** — lying contracts mislead contributors; honest contracts onboard them. This project is OSS-bound; contributor onboarding is a first-class structural concern, not a postscript.
