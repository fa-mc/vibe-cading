...
**Symptom:** A recess, counterbore, or pocket that should sink *into* a part sits in open air just outside the entry face and removes almost nothing — the shoulder never forms and the bore stays at shaft diameter for the full wall, leaving no clearance for the head. The failure is always **under**-removal: on the wrong side of the entry face the recess cannot cut deeper than intended. The part still builds, still passes `assert len(result.solids().vals()) == 1`, and still looks right in every external view, because the missing material is on the *inside*.

**Root cause:** `cq.Workplane.extrude(d)` extrudes along the workplane's **+normal** (+Z for `"XY"`) when `d` is positive, and that default is invisible at the call site: `cq.Workplane("XY", origin=(0, 0, z_entry)).circle(r).extrude(depth)` *reads* as "a recess of `depth` at `z_entry`" but spans `[z_entry, z_entry + depth]` — the wrong side whenever the material to remove lies *below* the entry face. It hides best when the origin is **offset** rather than at the workplane's own Z=0 (e.g. `z_recess = -tolerance.free.axial`): attention goes to computing the entry Z, and the direction is never stated at all.

**Fix:**
1. **State the direction explicitly whenever the origin is offset.** Anchor the origin at the *far* end and extrude back toward the entry face — `cq.Workplane("XY", origin=(0, 0, z_entry - depth)).circle(r).extrude(depth)` — or extrude a negative amount. Never lean on the default when the entry face is not the workplane's own Z=0.
...
3. **Assert WHERE the recess is, not merely THAT it exists.** A recess floating outside the part still has the right radius and volume — only its position is wrong — so any check blind to location passes it (*A Check That Cannot Fail Is Not A Check*, §4). A presence probe is fine **provided its location is one only correct geometry can occupy**. Choose it by what else inflates the bbox: `zmax` suffices on a `.solid`, but on a `.to_cutter()` the 100 mm through-overcut swamps `zmax` whichever way the recess points, so probe inside the recess annulus instead.

*(Triggering incident: `CounterboreHole` (`vibe_cading/mechanical/holes.py`) carried this **twice, independently** — in `.solid` and again in `.to_cutter()` — until PR #94 gave the cylinder branch its first real consumer. One class making the identical slip in two methods is what marks it a footgun, not a typo. Guards: `tests/test_cutter_overcut.py::test_counterbore_hole_cylinder_head_sinks_into_part` (bbox) and `::test_counterbore_hole_to_cutter_cylinder_head_sinks_into_part` (annulus probe) — the two shapes in Fix 3.)*
