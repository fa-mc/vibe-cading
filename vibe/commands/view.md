...
Per `vibe/INSTRUCTIONS.md` ("OCP Viewer — Dedicated Entry Point"), model class files must remain pure declarations — never add `ocp_vscode` imports or `if __name__ == "__main__":` viewer blocks to them. `tools/view.py` is the only supported entry point.
