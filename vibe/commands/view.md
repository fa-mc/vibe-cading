---
description: Open a model in the OCP CAD viewer (port 3939) via vibe_cading/tools/view.py
...
Per `vibe/INSTRUCTIONS.md` ("OCP Viewer — Dedicated Entry Point"), model class files must remain pure declarations — never add `ocp_vscode` imports or `if __name__ == "__main__":` viewer blocks to them. `vibe_cading/tools/view.py` is the only supported entry point.
