...
- [ ] Clean up doc nits deferred (non-blocking) from PR #94's 3-round isolated review: `vibe_cading/rc/arrma_223s_esc_mount.py`'s module docstring resize-note item 2 still cites the pre-derivation `HOLE1_CENTER` X value (1.026) instead of the shipped `-3.5`; `docs/design_plans/2026-08-31-arrma-223s-receiver-mount_design.md`'s Tests-table rows 1-2 still name the pre-rename `ArrmaReceiverMount` class; `CHANGELOG.md`'s `CounterboreHole` Fixed entry still implies an ear pan-head recess the 2026-09-01 resize removed. Also confirm with the human whether the south ear hole's 4.53 mm X-shift (now tracking `MOTOR_HOLE_X` instead of its originally-measured 1.026) against the physical part is intended — flagged by `tl` review as plausible but unverified against hardware.
