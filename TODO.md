...
- [ ] Exclude `vibe_cading/mcp/` from the `engine_api.json` class catalog at the extractor level. `extract_classes` (`vibe_cading/tools/engine_api/extractor.py`) walks all of `vibe_cading/**`, so any **public** class added under `vibe_cading/mcp/` would wrongly appear in the engine class catalog and fail `gen_engine_api.py --check`. Currently mitigated only by keeping the MCP package's internal exception underscored (`_ToolError`) — a workaround, not a guard. Introduce a real exclusion seam (an `exclude=` parameter / per-class guard on `extract_classes`; note `experiments/` is excluded merely by omission from the roots list in `gen_engine_api.py`, not by a subtree filter) **and** land a regression test asserting no `vibe_cading.mcp.*` fqn appears in `engine_api.json` (mirror the existing `*Protocol`-leak gate at `extractor.py:281`). Deferred from the RFC #41 MCP PR — TL Phase-B finding TL-1 (out-of-scope shared-`tools/` change; the underscore keeps `--check` green meanwhile). — **Addressed in [PR #48](https://github.com/fa-mc/vibe-cading/pull/48)** (commit `67d89d3`): `exclude=` param on `extract_classes` + committed-artifact leak gate + non-vacuous control/treatment efficacy test; `engine_api.json` byte-stable; 5-lens adversarial verification all PASS. Flip to done on merge.
