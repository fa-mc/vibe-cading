...
- [ ] Consolidate blind bearing-pocket sizing: `FreespinHexHub`, `HexHubNut` (PR #88), and `Bearing.outer_pocket` each compute a bearing-pocket diameter/depth (`bearing_od/width + fit.radial/axial [+ 0.5 proud margin]`) independently rather than sharing one implementation. Add a blind-pocket variant on `Bearing` (or a `cq_utils` helper) that all three call, and centralize the duplicated `MR85_OD`/`MR85_W` = 8.0/2.5 constants (currently defined separately in `freespin_hex_hub.py`, `bearing_hex_housing.py`, and `hex_hub_nut.py`) alongside the `Bearing` class. Flagged by independent `tl` review in PR #88's isolated review cycle.
