...
- [ ] Teach `vibe_cading/tools/check_visual_contract_freshness.py` to understand cross-class assembly-module rows (an `assemble()` function composing multiple `.solid`s, e.g. `vibe_cading/lego_adapters/poweredup_hub/assembly.py`), so a combined-seated-view SVG can be registered and freshness-checked like a single-class contract instead of living only as an allowlist exemption. Deferred from the PoweredUp hub battery-box implementation (TL phase-4 review, finding M4): the exempt `_design_iso_ne.svg` for the assembly view is a tracked file regenerated by nothing today. Documented at three sites meanwhile: the checker's exemption comment, `visual_contracts.toml`, and `assembly.py`'s own docstring.
