...
- [ ] Exclude non-geometry helper classes under `vibe_cading/tools/` from the `engine_api.json` catalog. `extract_classes` walks all of `vibe_cading/**`, so tool-internal dataclasses already leak into the published wire contract with a nonsense `.solid` result accessor: `check_visual_contract_freshness.Contract`, and `engine_api.extractor.{ClassRecord,Constructor,Param}`. The `exclude=` seam added by [PR #48](https://github.com/fa-mc/vibe-cading/pull/48) for `vibe_cading/mcp/` is the mechanism — this is the same class of leak, one directory over. Currently mitigated per-file by underscoring (`fetch_reference._LDrawArchive`), which is a convention, not a guard. Removing the four existing entries is a **public wire-contract removal**, so it needs a version bump and TL review — deliberately out of scope for the `fetch_reference.py` PR that surfaced it.
