...
- [x] Consolidate blind bearing-pocket sizing: `FreespinHexHub`, `HexHubNut` (PR #88), and `Bearing.outer_pocket` each computed a bearing-pocket diameter/depth independently rather than sharing one implementation. Flagged by independent `tl` review in PR #88's isolated review cycle. — Addressed: added `Bearing.blind_pocket_dims()` (static method) and `Bearing.mr85()` preset; `FreespinHexHub`/`HexHubNut` now delegate to the shared formula (regression-tested behavior-identical), and the `MR85_ID`/`MR85_OD`/`MR85_W` constants (previously duplicated in three files) now live once on `bearings.py`. `Bearing.outer_pocket` was deliberately **not** folded into the same helper — it builds a *through-cut* solid with flange support and no proud margin, a genuinely different shape/formula from the blind-pocket case, not just a naming inconsistency.
...
- [ ] ~~Clean up doc nits deferred (non-blocking) from PR #94's 3-round isolated review~~ — the three doc nits (stale `HOLE1_CENTER` docstring value, stale `ArrmaReceiverMount` name in the design doc's Tests table, stale `CHANGELOG.md` wording) were fixed in [PR #95](https://github.com/fa-mc/vibe-cading/pull/95). Still open: confirm with the human whether the south ear hole's 4.53 mm X-shift (now tracking `MOTOR_HOLE_X` instead of its originally-measured 1.026) against the physical part is intended — flagged by `tl` review as plausible but unverified against hardware.
- [ ] **`ToleranceGauge`'s printed geometry silently changed in PR #94, and its own changelog entry says it didn't.** PR #94's `CounterboreHole` Z-direction fix (`vibe_cading/mechanical/holes.py`) claimed "no shipped model previously exercised this code path" / "no other model's geometry changes as a result" — both false. `ToleranceGauge` (`vibe_cading/mechanical/tolerance_gauge.py`) drives an M3 socket-head cutter through the exact same `head_type != "flat"` → `CounterboreHole(head_type="cylinder")` branch, and its volume measurably changed as a result (31573.8345 mm³ before PR #94's fix -> 31327.2587 mm³ after, a -246.58 mm³ delta, independently verified with a branch control on 2026-09-05). This slipped through because `ToleranceGauge` is registered in neither `build.toml` nor `visual_contracts.toml` — no build target and no visual contract covered it. `ToleranceGauge` is the physical part users print to calibrate their printer's tolerances, so anyone who printed it before vs. after PR #94 has two different parts. [PR #95](https://github.com/fa-mc/vibe-cading/pull/95) corrected the false claim in `CHANGELOG.md`'s text; this item is the still-open follow-up: (1) confirm the corrected (post-fix) recess geometry is actually right for the gauge's intended function, since nobody has looked at it from that angle yet; (2) decide whether `ToleranceGauge` should get `build.toml` / `visual_contracts.toml` registration so a future silent geometry change to a shared `mechanical/` primitive can't slip past this same gap again.
- [x] Exclude `vibe_cading/mcp/` from the `engine_api.json` class catalog at the extractor level. `extract_classes` (`vibe_cading/tools/engine_api/extractor.py`) walks all of `vibe_cading/**`, so any **public** class added under `vibe_cading/mcp/` would wrongly appear in the engine class catalog and fail `gen_engine_api.py --check`. Currently mitigated only by keeping the MCP package's internal exception underscored (`_ToolError`) — a workaround, not a guard. Introduce a real exclusion seam (an `exclude=` parameter / per-class guard on `extract_classes`; note `experiments/` is excluded merely by omission from the roots list in `gen_engine_api.py`, not by a subtree filter) **and** land a regression test asserting no `vibe_cading.mcp.*` fqn appears in `engine_api.json` (mirror the existing `*Protocol`-leak gate at `extractor.py:281`). Deferred from the RFC #41 MCP PR — TL Phase-B finding TL-1 (out-of-scope shared-`tools/` change; the underscore keeps `--check` green meanwhile). — **Addressed in [PR #48](https://github.com/fa-mc/vibe-cading/pull/48)** (commit `67d89d3`): `exclude=` param on `extract_classes` + committed-artifact leak gate + non-vacuous control/treatment efficacy test; `engine_api.json` byte-stable; 5-lens adversarial verification all PASS. **Done — PR #48 merged as `6c45f13` (admin-merged 2026-06-18, CI green).**
...
- ~~**Branch protection on `main` — apply at the public flip (root-cause fix for [#40](https://github.com/fa-mc/vibe-cading/issues/40)).**~~ — **DONE 2026-06-17** (repo went public same day). Active ruleset **"main protection"** (`id 17812480`, `enforcement: active`) on the default branch: require-PR (1 approving review) + **required status check `CLAAssistant`** (the CLA Assistant *Lite* Actions job, `integration_id 15368`) + `non_fast_forward` (blocks force-push) + `deletion` (blocks deleting `main`). Bypass = Admin role (`bypass_mode: always`) → the maintainer keeps the `TODO.md` direct-push carve-out while external contributors are gated (never admins). Closes the #40 root cause: unsigned external code can no longer merge to `main`. The earlier private-repo blocker (rulesets unenforced on Free/personal; GitHub steering to a Team org) was resolved by going public — public repos enforce rulesets for free. *(Caution carried forward: anyone granted the Admin role bypasses the ruleset — keep Admin grants tight; use Write/Maintain for gated collaborators.)*
