...
- ~~**Purge non-publishable docs from working tree AND git history.**~~ — **DONE 2026-05-28**. Non-publishable internal docs (a confidential commercial plan and private downstream design notes) were removed from the working tree and purged from all git history via `git filter-repo --invert-paths` (the two commits that introduced them were rewritten/pruned), then the rewrite was published to `origin/main` from a fresh-clone. Ship/no-ship audit of remaining `docs/` complete: `templates/design-brief-template.md`, `agentic-workflow.md`, `lego-technic.md`, `screws.md`, `print-tolerances.md` all SHIP. An off-clone pre-purge mirror is retained by the maintainer for one transition window.

### Deferred features (gated on real demand)

- **Pip-installable distribution + PyPI publication.** Deferred 2026-05-14 by Admin diagnosis: Option C (pip-installable) was a single-adapter speculative seam — one hypothetical downstream consumer doesn't justify HE Phase 1 effort + ongoing PyPI maintenance. The renamed `vibe_cading/` namespace is structurally pip-ready (a future PR adds `pyproject.toml` + `[tool.setuptools.packages.find].exclude` for `parts`/`experiments` + version metadata; ships to PyPI). **Trigger condition to revisit:** a second real consumer is committed (NOT hypothetical — a downstream actively asking for `pip install vibe-cading`). Until then, distribution = `git clone`.
