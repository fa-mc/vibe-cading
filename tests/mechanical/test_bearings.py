...
def test_freespin_hex_hub_pocket_dims_match_shared_formula() -> None:
    ...
    expected = Bearing.blind_pocket_dims(9.0, 3.0, profile=get_profile())
    assert hub._pocket_dia == pytest.approx(expected[0])
    assert hub._pocket_depth == pytest.approx(expected[1])


def test_hex_hub_nut_pocket_dims_match_shared_formula() -> None:
    nut = HexHubNut(bearing_od=9.0, bearing_width=2.0)
    expected = Bearing.blind_pocket_dims(9.0, 2.0, profile=get_profile())
    assert nut._pocket_dia == pytest.approx(expected[0])
    assert nut._pocket_depth == pytest.approx(expected[1])
